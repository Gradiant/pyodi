...
from pyodi.coco.utils import get_bbox_matrix, scale_bbox_dimensions


@pytest.fixture
def get_simple_annotations_with_img_sizes():
    def _get_fake_data():
        bboxes = np.array([[0, 0, 10, 10, 100, 100], [25, 25, 50, 50, 100, 100]])
        columns = [
            "col_centroid",
            "row_centroid",
            "width",
...
def test_scale_bbox_dimensions(get_simple_annotations_with_img_sizes):
    ...
    expected_bboxes = np.array([[0, 0, 128, 72], [320, 180, 640, 360]], dtype=np.int32)
